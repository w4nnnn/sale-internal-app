...
export function DetailPelangganDialog({ open, onOpenChange, pelanggan }) {
	const [licenses, setLicenses] = useState([]);
	const [isLoadingLicenses, setIsLoadingLicenses] = useState(false);

	useEffect(() => {
		if (!open || !pelanggan?.pelanggan_id) {
			setLicenses([]);
			return;
		}
		...
		const fetchLicenses = async () => {
			...
			try {
				const response = await fetch(`/api/pelanggan/${pelanggan.pelanggan_id}/licenses`);
				if (!response.ok) {
					...
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<DialogDescription className="text-muted-foreground">
								Informasi lengkap tentang pelanggan ini.
							</DialogDescription>
						</div>
					</div>
				</DialogHeader>
							...
							<div className="grid gap-2 sm:grid-cols-2">
								<div className="flex items-start gap-3 p-3 rounded-lg bg-muted/30">
									...
									<div className="flex-1">
										...
										<dd className="text-sm text-foreground font-medium">{pelanggan.nama_pelanggan}</dd>
									</div>
								</div>
								<div className="flex items-start gap-3 p-3 rounded-lg bg-muted/30">
									<div className="p-1.5 bg-background rounded-md">
									...
									<div className="flex-1">
										...
										<dd className="text-sm text-foreground">
											{pelanggan.email_pelanggan || <span className="text-muted-foreground">-</span>}
										</dd>
									</div>
								</div>
								<div className="flex items-start gap-3 p-3 rounded-lg bg-muted/30">
									...
									<div className="flex-1">
										...
										<dd className="text-sm text-foreground">
											{pelanggan.perusahaan || <span className="text-muted-foreground">-</span>}
										</dd>
									</div>
								</div>
								<div className="col-span-2 flex items-start gap-3 p-3 rounded-lg bg-muted/30">
									...
									<div className="flex-1">
										...
										<dd className="text-sm text-foreground">
											{pelanggan.telepon_pelanggan || <span className="text-muted-foreground">-</span>}
										</dd>
									</div>
								</div>
							</div>
						...
						<CardContent>
							{isLoadingLicenses ? (
								...
								<div className="p-4 bg-muted/30 rounded-lg text-center">
									...
									<p className="text-muted-foreground text-sm">Tidak ada aplikasi yang dimiliki.</p>
								</div>
							) : (
											...
											<div className="flex items-start justify-between">
												<div className="flex-1">
													...
													<p className="text-xs text-muted-foreground mb-2">
														User: <span className="font-medium">{license.nama_user}</span>
													</p>
